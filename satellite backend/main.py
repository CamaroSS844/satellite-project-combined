"""
Satellite Realignment Backend — Full Rewrite
Includes: Weather API, SQLite persistence, PDF generation, all endpoints
"""
...
# Static station coordinates (can be overridden via env vars or config)
STATION_COORDS = {
...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS kpi_log (
            ...
            ts                 TEXT,
            avg_signal_dbm     REAL,
            realignments_hour  REAL,
            downtime_reduction REAL,
            power_usage_w      REAL
        )
    """)
    con.commit()
...
def persist_env(station_id: str, w: WeatherData):
    ...
    db_exec(
        ...
        (station_id, ts, w.temperature, w.wind_speed, w.rain, w.humidity, w.pressure)
    )
...
# APP SETUP
# ─────────────────────────────────────────────────────────────────────────────
app = FastAPI(title="Satellite Realignment Backend v2")
...
def update_angles(data: AngleUpdateIn):
    ...
    if station.command.pending and station.target_angles:
        ...
        if da <= ANGLE_TOLERANCE and de <= ANGLE_TOLERANCE:
            ...
            log_to_db(station.station_id, "Manual realignment completed")
...
async def environmental_data(loc: UserLocationIn):
    """Returns weather for both stations + optional user location."""
...
def _build_pdf() -> bytes:
    ...
    story.append(Paragraph("Operational Report", styles["Heading2"]))
    ...
    overview_data = [
        ...
        ["Backend Mode",        "Authoritative"],
        ["Architecture",        "ESP32 → MQTT → FastAPI → Dashboard"],
    ...
    t2 = Table(rows, colWidths=[4.5*cm, 2*cm, 2*cm, 2.5*cm, 2.5*cm, 2.5*cm])
    ...
    story.append(Paragraph("3. Environmental Data (Latest Readings)", h1))
    ...
    env_table = [["Station", "Timestamp", "Temp (°C)", "Wind (km/h)", "Rain (mm/h)", "Humidity (%)", "Pressure (hPa)"]]
    ...
    if len(env_table) == 1:
        env_table.append(["No data yet", "-", "-", "-", "-", "-", "-"])
    t3 = Table(env_table, colWidths=[3.8*cm, 3*cm, 2*cm, 2.2*cm, 2.4*cm, 2.4*cm, 2.2*cm])
    ...
    # ── 4. KPIs ────────────────────────────────────────────────────────────
    story.append(Paragraph("4. Key Performance Indicators", h1))
    kpi_rows_db = db_fetch(
        "SELECT ts, avg_signal_dbm, realignments_hour, downtime_reduction, power_usage_w FROM kpi_log ORDER BY id DESC LIMIT 10"
    )
    kpi_table = [["Timestamp", "Avg Signal (dBm)", "Realignments", "Downtime Red. (%)", "Power (W)"]]
    for row in kpi_rows_db:
        kpi_table.append([
            row["ts"][:16],
            f"{row['avg_signal_dbm']:.1f}",
            f"{row['realignments_hour']}",
            f"{row['downtime_reduction']:.1f}",
            f"{row['power_usage_w']:.1f}",
        ])
    if len(kpi_table) == 1:
        kpi_table.append(["No data yet", "-", "-", "-", "-"])
    t4 = Table(kpi_table, colWidths=[4*cm, 3.5*cm, 3*cm, 3.5*cm, 3*cm])
    ...
    # ── 5. Alerts & Logs ──────────────────────────────────────────────────
    story.append(Paragraph("5. Alerts & System Logs (Last 30)", h1))
    log_rows = db_fetch(
    ...
    # ── 6. Trends Summary ─────────────────────────────────────────────────
    story.append(Paragraph("6. Trends Summary", h1))
    env_all = db_fetch("SELECT station_id, temperature, wind_speed, humidity, pressure FROM env_log")
    if env_all:
        temps = [r["temperature"] for r in env_all]
        winds = [r["wind_speed"]  for r in env_all]
        story.append(Paragraph(
            f"Temperature range across all stations: {min(temps):.1f}°C – {max(temps):.1f}°C "
            f"(avg {sum(temps)/len(temps):.1f}°C)", body
        ))
        story.append(Paragraph(
            f"Wind speed range: {min(winds):.1f} – {max(winds):.1f} km/h "
            f"(avg {sum(winds)/len(winds):.1f} km/h)", body
        ))
    else:
        story.append(Paragraph("Insufficient environmental data for trend analysis.", body))

    story.append(Spacer(1, 0.6*cm))
    story.append(HRFlowable(width="100%", thickness=0.5, color=colors.grey))
    story.append(Paragraph("End of Report — Satellite Realignment System", small))
...
async def heartbeat_monitor():
    """Mark stations offline when heartbeat times out."""
    while True:
...
async def startup():
    ...
    log_to_db("system", "Backend started")
